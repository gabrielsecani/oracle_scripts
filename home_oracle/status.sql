...
select INSTANCE_NUMBER, status, DATABASE_STATUS, INSTANCE_ROLE, ACTIVE_STATE, BLOCKED
  from v$instance;

PROMPT 
PROMPT ## Database Status
SELECT DB_UNIQUE_NAME, NAME, OPEN_MODE, DATABASE_ROLE, SWITCHOVER_STATUS
  FROM v$database;

select DB_UNIQUE_NAME, DBID, FLASHBACK_ON, protection_mode, protection_level
  from v$database;
